...
func (cr *CurrencyRepository) GetCurrencyByCode(ctx context.Context, code string) (*entity.Currency, error) {
	db := cr.db.GetTx(ctx)

	var currency model.Currency
	if err := db.WithContext(ctx).Where("code = ?", code).First(&currency).Error; err != nil {
		if cr.db.ErrRecordNotFound(ctx, err) {
			logger.Warn(ctx, "Currency not found with code", code)
			return nil, nil
		}
		logger.Error(ctx, "Error finding currency by code", err.Error())
		return nil, err
	}
